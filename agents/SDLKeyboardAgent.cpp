...
Action SDLKeyboardAgent::act() {
  if (manual_control) {
    ...
    //hbj return choice(&available_actions);
	return waitForKeypress(); //hbj
}
...
Action SDLKeyboardAgent::waitForKeypress() {
    ...
    //hbj while (a == UNDEFINED) 
	{
        //hbj SDL_Delay(50); // Set amount of sleep time
        SDL_PumpEvents();
        ...
        if (keymap[SDLK_p]) {
            ...
            // Triple Actions
        } else if (keymap[SDLK_UP] && keymap[SDLK_RIGHT] && keymap[SDLK_f]) {
            a = PLAYER_A_UPRIGHTFIRE;
        } else if (keymap[SDLK_UP] && keymap[SDLK_LEFT] && keymap[SDLK_f]) {
            a = PLAYER_A_UPLEFTFIRE;
        } else if (keymap[SDLK_DOWN] && keymap[SDLK_RIGHT] && keymap[SDLK_f]) {
            a = PLAYER_A_DOWNRIGHTFIRE;
        } else if (keymap[SDLK_DOWN] && keymap[SDLK_LEFT] && keymap[SDLK_f]) {
        ...
        } else if (keymap[SDLK_UP] && keymap[SDLK_f]) {
            a = PLAYER_A_UPFIRE;
        } else if (keymap[SDLK_DOWN] && keymap[SDLK_f]) {
            a = PLAYER_A_DOWNFIRE;
        } else if (keymap[SDLK_LEFT] && keymap[SDLK_f]) {
            a = PLAYER_A_LEFTFIRE;
        } else if (keymap[SDLK_RIGHT] && keymap[SDLK_f]) {
            a = PLAYER_A_RIGHTFIRE;

            // Single Actions
        } else if (keymap[SDLK_f]) {
            a = PLAYER_A_FIRE;
	...
	//hbj
	if(a == UNDEFINED) a = PLAYER_A_NOOP;
    return a;
}
#endif
