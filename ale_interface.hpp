...
static void createOSystem(int argc, char* argv[],
	...
	// hbj: disable time-dependent random seed, for experimental purpose
...
public:
	...
	// hbj: this routine is modified in order to send more arguments to the system 
    void loadROM(string rom_file, int frame_skip) {
        int argc = 8;
        ...
        strcpy(argv[4],(display_active)?"true":"false");
        
		strcpy(argv[5],"-frame_skip");
		sprintf(argv[6],"%d", frame_skip);

		strcpy(argv[argc-1],rom_file.c_str());  
